...
class MeshBridge {
  ...
  log(...a) { if (MeshBridge.DEBUG) this.app.log('[MeshBridge]', ...a); }
  ...
  _rxKey() {
    // Diagnostic: derive from the confirmed passphrase 1234.
    const d = crypto.createHash('sha256').update(Buffer.from('1234\x00MCP', 'utf8')).digest();
    ...
    try {
      ...
      for (let start = 0; start <= Math.max(0, frame.length - 14); start += 1) {
        for (let end = frame.length; end >= start + 14; end -= 1) {
          ...
          const iv = Buffer.alloc(16); seq.copy(iv, 0); src.copy(iv, 4);
          const dec = crypto.createDecipheriv('aes-128-ofb', key, iv);
          dec.setAutoPadding(false);
          const chex = Buffer.concat([dec.update(payload), dec.final()]).toString('hex');
  ...
  isActive() {
    return !!this._peripheral || !!this._connecting;
  }
  ...
  _statusPollMs() {
    ...
    // 30s default. A stored 0 / blank / garbage can no longer switch status off.
  ...
  _kickTxWorker() {
    if (this._txWorkerPromise) return;
  ...
  async _candidates() {
    ...
    let advs = [];
    try {
      advs = await this.homey.ble.discover();
    } catch (err) {
      this.error(`discover failed: ${err.message || err}`);
    }

    const sg = advs
      .filter((a) => SG_NAME.test(String(a.localName || '').trim()) && a.connectable !== false)
    ...
    for (const adv of sg) this._advCache.set(adv.uuid, adv);
  ...
  async _findPreferredAdvertisement(label = 'preferred') {
    ...
    try {
      ...
      if (adv && adv.connectable !== false) {
        this._advCache.set(adv.uuid, adv);
    ...
    this._connecting = (async () => {
      ...
      while (Date.now() < deadline) {
        ...
        if (fastPreferred && this._preferredUuid) {
          const cached = this._advCache.get(this._preferredUuid);
          if (cached) {
            fastCacheTried = true;
            cands = [cached];
            ...
            if (!refreshed) {
              ...
              const sg = advs.filter((a) => SG_NAME.test(String(a.localName || '').trim()) && a.connectable !== false);
              for (const adv of sg) this._advCache.set(adv.uuid, adv);
        ...
        for (const adv of cands) {
          ...
          try {
            ...
            if (fastPreferred && this._preferredUuid && adv.uuid === this._preferredUuid) {
              const cached = this._advCache.get(this._preferredUuid);
              if (cached === adv) this._advCache.delete(this._preferredUuid);
            }
  ...
  async _recoverPreferredTxConnection() {
    ...
    this._txRecoveryPromise = (async () => {
      ...
      if (this._preferredUuid) this._advCache.delete(this._preferredUuid);
      ...
      for (let attempt = 1; attempt <= RECOVERY_DISCOVER_ATTEMPTS; attempt += 1) {
        ...
        if (!preferred) {
          ...
          sg = advs.filter((a) =>
            SG_NAME.test(String(a.localName || '').trim()) && a.connectable !== false
          );
          for (const adv of sg) this._advCache.set(adv.uuid, adv);
        ...
        if (!preferred) {
          ...
          this._advCache.set(preferred.uuid, preferred);
          ...
          try {
            ...
            lastError = err;
            this._advCache.delete(preferred.uuid);
  ...
  async _ensureTxConnected() {
    ...
    const cachedPreferred = this._preferredUuid
      ? this._advCache.get(this._preferredUuid)
      : null;
  ...
  async _disconnect(reason = 'local') {
    ...
    const wasNotifySubscribed = this._notifySubscribed;
    const connNo = this._connNo;
    ...
    if (wasNotifySubscribed && char8004 && typeof char8004.unsubscribeFromNotifications === 'function') {
      try {
        await char8004.unsubscribeFromNotifications();
        this.log(`GATT unsubscribe 8004 OK #${connNo}`);
      } catch (err) {
        this.error(`GATT unsubscribe 8004 FAIL #${connNo}: ${err.message || err}`);
      }
    }
    if (peripheral) {
      try {
        ...
        if (/Peripheral Not Found/i.test(String(err.message || err)) && this._preferredUuid) {
          this._advCache.delete(this._preferredUuid);
  ...
  // Legacy hook retained as a no-op. TX idle handling is done by
...
MeshBridge.DEBUG = true; // v1.0.26: post-GATT freshness gate + retained latest-wins retry worker
